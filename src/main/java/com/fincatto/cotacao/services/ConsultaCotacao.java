...
public class ConsultaCotacao {
    ...
    public Cotacao getCotacao(final Moeda moeda, final LocalDate data) throws IOException {
        final URL url = new URL(String.format("https://olinda.bcb.gov.br/olinda/servico/PTAX/versao/v1/odata/CotacaoMoedaDia(moeda='%s',dataCotacao='%s')?$top=1&$orderby=dataHoraCotacao+desc&$format=json&$select=cotacaoCompra,cotacaoVenda", moeda, FORMATTER.format(data)));
